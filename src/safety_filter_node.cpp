...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// init_and_solve_ecos  LINEAR ALPHA COST
	//...........................................................................
	...
	double alpha_high = 10000; 
	double alpha_low = 0.25;
	double alphaB_low = 10; 
	double L_ah[2] = {1.0, 1.6724};
	double L_Lfh[2] = {3.664, 0.42};
	double L_Lgh[npBTSS+1]; 
	int assume = 0; // set to 0 to assume error only in position, sets Lgh_t to Lfh_t
	...
	for(int i=0; i<npBTSS; i++){
		L_Lgh[i] = assume*0.0081*integration_dt_*200.0*i/npBTSS;
	}	
	L_Lgh[npBTSS] = 0.0255; 
	...
	for (int i = 0; i < (int) npBTSS+1; i++)
	{
		...
		if (i<npBTSS)
		{
			h_ecos[7 + i*2] = (*asif).Lfh_out_[i] - assume*epsilon*L_Lfh[0]*integration_dt_*200.0*i/npBTSS; 
		}else{
			h_ecos[7 + i*2] = (*asif).Lfh_out_[i]  - epsilon*L_Lfh[1];
		}
		// ROS_INFO("%f", asif->Lgh_out_[i][0]);
	}
	//ROS_INFO("%f", asif->Lgh_out_[0][0]);
	...
	Gpr[3 + 2*(npBTSS+1) + 2 + npBTSS + 2] = -(*asif).h_out_[npBTSS]+epsilon*L_ah[1];
