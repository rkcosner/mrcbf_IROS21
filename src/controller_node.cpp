...
void computeControlAction(void)
{
	...
	double thetaDotDes = cmdCurrent_.cmd[1];

	const double u = gainsVec_.dot(stateCurrentVec_ - stateDes);
	double Kthetadot = 10;

	input_.inputVec[0] = u+Kthetadot*(stateCurrentVec_[4]-thetaDotDes);
	input_.inputVec[1] = u-Kthetadot*(stateCurrentVec_[4]-thetaDotDes);
}
